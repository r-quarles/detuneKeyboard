...
function setup() {
  ...
  for (let i = 0; i < buttonCount; i++) {
    ...
    if (i % 2 == 0) {  //If its an even key (white)
      circleY[i] = 2 * windowHeight / 3;               // Default y position for each button (2/3 down)
      ...
      circleY[i] = (2 * windowHeight / 3) - diameter*2;// Default y position for each black button (1/2 length of the slider up)
...
function draw() {
  ...
  text("Equal", controlButtonX[0]-0.5*diameter, controlButtonY[0]-diameter/7, diameter);
  text("Just", controlButtonX[1]-0.5*diameter, controlButtonY[1]-diameter/7, diameter);
  text("Pythagorean", controlButtonX[2]-0.5*diameter, controlButtonY[2]-diameter/7, diameter);
  text("QuarterComma", controlButtonX[3]-0.5*diameter, controlButtonY[3]-diameter/7, diameter);
...
function isClickingControl() {
  return dist(controlButtonX[buttonIndex], controlButtonY[buttonIndex], mouseX, mouseY) < diameter / 2;
}

// To update key positions when tuning is changed
function updateKeys(tuning){
  if (tuning = 0) {
    for (let i = 0; i< buttonCount; i++) {
      equalTuning(i);  //Assign each buttons frequency to the cooresponding 12TET ref freq
      centsCalc(i);  //Send centsCalc which button and it will calculate the cents between that buttons new freq and the ref freq in EDO
